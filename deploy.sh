...
RUNNING_CONTAINERS=$(docker compose --env-file .env.prod ps -q app)

# 3. 각 컨테이너를 순회하며 하나씩 업데이트
for CONTAINER_ID in $RUNNING_CONTAINERS; do
  CONTAINER_NAME=$(docker inspect --env-file .env.prod --format='{{.Name}}' $CONTAINER_ID | sed 's/^\///')
  ...
  UPSTREAM_CONFIG=$(docker compose --env-file .env.prod ps -q app | awk '{print "server " $1 ":3000 " ($1 == "'$CONTAINER_ID'" ? "down" : "") ";"}')
  ...
  docker compose --env-file .env.prod up -d --no-deps --force-recreate --no-build $CONTAINER_NAME
  ...
  UPSTREAM_CONFIG=$(docker compose --env-file .env.prod ps -q app | awk '{print "server " $1 ":3000;"}')
  echo "upstream reflex_app { ${UPSTREAM_CONFIG} }" > ./nginx/conf.d/upstream.conf
  docker compose --env-file .env.prod exec nginx nginx -s reload
done
